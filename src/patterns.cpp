...
void TwinkleRandom(int Count, int SpeedDelay, boolean OnlyOne, int max) {
    ...
    for (int i = 0; i < Count; i++) {
        setPixel(random(NUM_LEDS), random(0, max), random(0, max), random(0, max));
        showStrip();
        delay(SpeedDelay);
        if (OnlyOne) {
            setAll(0, 0, 0);
...
void SparkleXmas(byte red, byte green, byte blue, int SpeedDelay) {
    int Pixel = random(NUM_LEDS);
    if (Pixel % 2 == 0) {
        setPixel(Pixel, red, 0, 0);
    } else {
        setPixel(Pixel, 0, green, 0);
    }
    showStrip();
    delay(SpeedDelay);
    setPixel(Pixel, 0, 0, 0);
...
void theaterChaseRainbow(int SpeedDelay, int dimness) {
    ...
    for (int j = 0; j < 256; j++) {  // cycle all 256 colors in the wheel
        for (int q = 0; q < 3; q++) {
            for (int i = 0; i < NUM_LEDS; i = i + 3) {
                c = Wheel((i + j) % 255);
                setPixel(i + q, *c / dimness, *(c + 1) / dimness, *(c + 2) / dimness);  // turn every third pixel on
            }
            showStrip();
...
void display_status(int status) {
    ...
    delay(1);  // Need time to turn on
    bool red = status < 0 ? 1 : 0;

    for (int i = 0; i < abs(status); i++) {
        strip.setPixelColor(i, strip.Color(red * DIM, !red * DIM, 0));
    }
    strip.show();
